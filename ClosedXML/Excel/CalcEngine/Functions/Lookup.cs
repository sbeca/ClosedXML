...
namespace ClosedXML.Excel.CalcEngine.Functions
{
    internal static class Lookup
    {
        public static void Register(FunctionRegistry ce)
        {
            ...
            ce.RegisterFunction("XLOOKUP", 3, 6, AdaptXlookup(Xlookup), FunctionFlags.Range | FunctionFlags.Future, AllowRange.Only, 1, 2);
        }
        ...
        private static AnyValue Xlookup(CalcContext ctx, ScalarValue lookupValue, AnyValue lookupRangeValue, AnyValue returnRangeValue, ScalarValue ifNotFoundValue, int matchMode, int searchMode)
        {
            if (lookupValue.IsError)
                return lookupValue.ToAnyValue();
            ...
            // TODO: Implement using the different searchMode values
            if (searchMode == 0 || searchMode < -2 || searchMode > 2)
                return XLError.IncompatibleValue;

            if (matchMode == 0) // 0 - Try to find exact match. If none found, return ifNotFoundValue or #N/A if ifNotFoundValue is blank
            {
            ...
            else if (matchMode == -1) // -1 - Try to find exact match. If none found, return the next smaller item.
            {
                // Bisection in Excel and here differs, so we return different values for unsorted ranges, but same values for sorted ranges.
            ...
            else if (matchMode == 1) // 1 - Try to find exact match. If none found, return the next larger item.
            {
                // Bisection in Excel and here differs, so we return different values for unsorted ranges, but same values for sorted ranges.
            ...
            else if (matchMode == 2) // 2 - A wildcard match where *, ?, and ~ have special meaning: https://support.microsoft.com/en-us/office/using-wildcard-characters-in-searches-ef94362e-9999-4350-ad74-4d2371110adb
            {
                // TODO: Implement wildcard search
                return XLError.IncompatibleValue;
            }
